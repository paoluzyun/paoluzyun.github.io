...
def request_design(key: str, model: str, prompt: str) -> dict:
    payload = {
        "model": model,
        "messages": [
            {
    ...
    return clean_json(result["choices"][0]["message"]["content"])
    ...
    for attempt in range(1, 4):
        ...
        if validation_error:
            attempt_prompt += (
                "\n\n上次输出未通过动态接口检查："
                f"{validation_error}\n"
                "请保留你的设计自由，只修正缺失接口或安全问题，并重新输出完整 JSON。"
            )
        generated = request_design(key, model, attempt_prompt)
